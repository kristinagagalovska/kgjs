        ...
        <div class="col-md-3" id="menyMajtas">
            ...
            <ul class="nav nav-pills  nav-stacked">
                ...
                @foreach ($categories as $category)
                    <li role="presentation"><a href="{{route('t2.show', $category->title)}}">{{$category->title}}</a></li>
                    <li role="separator" class="divider"></li>
                    @foreach($subCat as $sub)
                        @if($sub->catId == $category->id)
                            <li role="presentation"><a href="{{route('sub2.show', ['id' => $category->id, 'title' => $sub->title])}}">{{$sub->title}}</a></li>
                        @endif
                    @endforeach
                    <li role="separator" class="divider"></li>
                @endforeach
