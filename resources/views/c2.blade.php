        ...
        <div class="col-md-3" id="menyMajtas">
            ...
            <ul class="nav nav-pills  nav-stacked">
                ...
                @foreach ($categories as $cate)
                    <li role="presentation"><a href="{{route('t2.show', $cate->title)}}">{{$cate->title}}</a></li>
                    <li role="separator" class="divider"></li>
                    @foreach($subCat as $sub)
                        @if($sub->catId == $cate->id)
                            <li role="presentation"><a href="{{route('sub2.show', ['id' => $cate->id, 'title' => $sub->title])}}">{{$sub->title}}</a></li>
                        @endif
                    @endforeach
                    <li role="separator" class="divider"></li>
                @endforeach
